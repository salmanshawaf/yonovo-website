...
| 2026-09-09 | none scanned | Scan could not run, fifth consecutive time. Fresh container, same result: proxy status clean (`enabled: true`, `recentRelayFailures: []`) yet `upflow.io`, `www.chaserhq.com` and `stuut.ai` all return `EGRESS_BLOCKED` via WebFetch. Ahrefs `upflow.io/blog/` prefix (US, 2026-09-09 vs 2026-09-02, `traffic_diff:desc`, 20 rows) returned **every row `status: "both"`** for the third independent run. Top gainer this week is `/blog/ar-metrics/net-credit-sales` (+232 to 349 US visits, "net credit sales" 1.1k/mo, pos 2), an established page, not a last-14-days post. Next gainers (`/ar-collections/late-fee` +51, `/ar-collections/outstanding-invoices` +22) likewise long-established. Zero new pages across three independent runs confirms Ahrefs is closed as a discovery substitute. | none | blocked: competitor sites unreachable |

## Known blockers

**2026-09-01 (still open as of 2026-09-09, 5 runs lost, 10 days with no post shipped): competitor
...
`"gateway answered 403 to CONNECT (policy denial or upstream failure)"`; on 2026-09-07 and again on
2026-09-09, from fresh containers with `recentRelayFailures: []`, the same three hosts were refused
identically. A clean proxy that still refuses exactly these hosts is conclusive: this is the
...
- **Ahrefs as a substitute for the index scan.** Ahrefs top-pages carries no publish date, and new
  posts have no traffic yet by definition, so they never surface as gainers. Confirmed on
  2026-09-05 (all 15 Upflow gainers `status: "both"`), 2026-09-07 (all 20 `status: "both"`) and
  2026-09-09 (all 20 `status: "both"`). Three independent runs, zero new pages: this avenue is
  closed, not unlucky. Stop re-testing it.
