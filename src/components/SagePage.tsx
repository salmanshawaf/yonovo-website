...
const comparisonFeatures = [
  { feature: "Invoice creation and tracking", sage: true, yonovo: true },
  { feature: "Aging reports", sage: true, yonovo: true },
  { feature: "Automated payment reminders", sage: false, yonovo: true },
  { feature: "Multi-channel follow ups (email, SMS, phone)", sage: false, yonovo: true },
  { feature: "AI-powered collection strategies", sage: false, yonovo: true },
  { feature: "Customer risk scoring", sage: false, yonovo: true },
  { feature: "Escalation workflows", sage: false, yonovo: true },
  { feature: "Real time AR dashboard", sage: false, yonovo: true },
  { feature: "Payment portal for customers", sage: false, yonovo: true },
  { feature: "DSO reduction tracking", sage: false, yonovo: true },
];
...
const timelineSteps = [
  {
    ...
    image: "/images/sage-step-1-invoice.png",
  },
  {
    number: 2,
...
const faqs = [
  {
    question: "How does Yonovo connect to Sage?",
    answer:
      "Yonovo connects to your Sage account through a secure API integration. Authorize access with your credentials and your data syncs automatically. No custom development needed.",
  },
  {
    question: "What Sage data does Yonovo access?",
    answer:
      "Yonovo syncs your invoices, customers, payment status, and aging data. Yonovo never modifies your Sage records. All data flows one way, from Sage to Yonovo, with payment status updates written back when collected.",
  },
  {
    question: "Which Sage products are supported?",
    answer:
      "Yonovo works with Sage Intacct, Sage 50, Sage 100, Sage 200, Sage 300, and Sage Business Cloud Accounting. If your version has an invoicing module, Yonovo can connect to it.",
  },
  {
    question: "How long does setup take?",
    answer:
      "Most teams are live within 15 minutes. Connect Sage, set your preferences, and Yonovo starts working on your overdue invoices right away.",
  },
  {
          ...
          <div className="flex flex-col gap-8 md:gap-16">
            <div className="flex flex-col gap-4 md:gap-6">
              ...
              <h1 className="text-balance font-medium text-[42px] text-white leading-[1.1] tracking-tight md:text-[70px]">
                Sync Sage and get paid
              </h1>
              <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
                <p className="text-balance text-base text-zinc-400 tracking-tight md:text-xl">
                  Yonovo connects to Sage and takes over the collections process. Every overdue invoice gets followed up by email, text, and phone so your team can stop spending hours on manual reminders.
                </p>
            ...
            <div className="flex flex-col gap-6 md:grid md:grid-cols-[2fr_3fr] md:gap-8">
              <VideoPlayer src="/videos/quickbooks-sync.mov" className="aspect-square w-full overflow-hidden rounded-2xl bg-zinc-900 border border-white/10" />
            ...
            <div className="flex w-full flex-col items-center gap-0">
              ...
              <div className="w-full -mt-5">
                <div className="flex items-center justify-center gap-10">
                  {logos.map((logo) => (
                    <Image key={logo.name} src={logo.src} alt={logo.name} width={logo.width} height={48} className={`w-auto ${logo.noFilter ? "opacity-60" : "brightness-0 invert opacity-60"} ${logo.className}`} />
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:gap-16">
            <div className="flex flex-1 flex-col gap-4">
              ...
              <h2 className="text-balance font-medium text-4xl text-foreground leading-tight tracking-tight md:text-[54px]">
                Sage wasn&apos;t made for collections
              </h2>
              <p className="text-balance text-base text-secondary leading-normal tracking-tight md:text-xl">
                Sage handles your accounting, payroll, and financial reporting. But once an invoice is sent, collecting payment still falls on your team. That gap costs you time, cash flow, and focus every single week.
              </p>
            </div>
          ...
          <div className="flex flex-col gap-8 md:gap-16">
            <div className="grid grid-cols-1 items-end gap-8 md:grid-cols-2 md:gap-16">
              <div className="flex flex-1 flex-col gap-4">
                ...
                <h2 className="text-balance font-medium text-4xl text-foreground leading-tight tracking-tight md:text-[54px]">
                  Yonovo picks up where Sage stops
                </h2>
              </div>
        ...
        <div className="mx-auto max-w-(--container-max-width) px-6">
          <div className="flex flex-col items-center gap-8 md:gap-14">
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <p className="text-base text-secondary leading-normal tracking-tight md:text-xl">See what your Sage setup is missing and what you unlock when you add Yonovo.</p>
            </div>
            <div className="hidden md:block overflow-hidden rounded-2xl border border-border bg-background max-w-3xl w-full">
              <div className="grid grid-cols-[1fr_140px_220px] border-b border-border">
                ...
                <div className="flex items-center justify-center border-l border-border px-4 py-4">
                  <Image src="/logos/sage-icon.svg" alt="Sage" width={80} height={26} className="h-5 w-auto" />
                </div>
                <div className="flex items-center justify-center gap-1.5 border-l border-brand-green/30 bg-brand-green/5 px-3 py-4">
                  <Image src="/logos/sage-icon.svg" alt="Sage" width={80} height={26} className="h-4 w-auto" />
              ...
              {comparisonFeatures.map((row, i) => (
                <div key={row.feature} className={`grid grid-cols-[1fr_140px_220px] ${i !== comparisonFeatures.length - 1 ? "border-b border-border" : ""}`}>
                  ...
                  <div className="flex items-center justify-center border-l border-border px-4 py-4">{row.sage ? <CheckIcon /> : <XIcon />}</div>
              ...
              {comparisonFeatures.map((row) => (
                <div key={row.feature} className="rounded-2xl border border-border bg-background p-4">
                  ...
                  <div className="flex items-center gap-6">
                    <div className="flex items-center gap-2">{row.sage ? <CheckIcon /> : <XIcon />}<span className="text-sm text-muted">Sage</span></div>
      ...
      <section className="w-full bg-gradient-to-b from-background from-60% to-brand-navy to-60%">
        <div className="mx-auto max-w-[1600px] px-6">
          <div className="relative flex w-full flex-col items-center justify-center gap-6 overflow-hidden border-t border-border bg-white px-6 py-12 pb-30 md:gap-8 md:rounded-3xl md:border md:p-15 lg:p-25">
            <div className="pointer-events-none absolute bottom-0 left-0 hidden md:block md:h-[40%] lg:h-[50%]"><div className="h-[350px] w-[350px] rounded-full bg-brand-blue opacity-70 blur-3xl" /></div>
            <div className="pointer-events-none absolute right-0 bottom-0 hidden md:block md:h-[40%] lg:h-[50%]"><div className="h-[350px] w-[350px] rounded-full bg-brand-blue opacity-70 blur-3xl" /></div>
